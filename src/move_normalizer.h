...
template <int N>
void
swap(std::shared_ptr<Bitboard<N>> board, SgPoint const & a, SgPoint const & b){
...
normalize_all_moves(std::vector<std::pair<std::shared_ptr<Bitboard<N>>, GoPlayerMove>> & moves) {
    ...
    // TODO den shared pointer nach aussen ziehen, also ueberall shared_ptr<pair<Bitboard<N>>,GoPlayerMove uebergeben
}
